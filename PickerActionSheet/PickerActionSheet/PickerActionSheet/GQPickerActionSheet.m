...
#define GQSystemVersion [[[UIDevice currentDevice] systemVersion] floatValue]

@interface PickerActionSheet()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>{
    NSInteger imageflag;//1:拍照 2:相册
    UIViewController *controllers;
}

@property (nonatomic, weak) id<PickerActionSheetDelegate> mdelegate;

@end

@implementation PickerActionSheet

- (void)showActionSheet:(UIViewController *)controller withDelegate:(id<PickerActionSheetDelegate>)delegate{
    controllers = controller;
    _mdelegate = delegate;
    if (GQSystemVersion >= 8) {
